...
public class RandomPersonCreationAppl {
	...
	private static List<Person> getRandomPersons() {
		return Stream.generate(RandomPersonCreationAppl::getRandomPerson)
				     .limit(N_PERSONS).collect(Collectors.toList());
	}
	private static Person getRandomPerson() {
		Person person = getRandomCommonPerson();
		return getChance() <= EMP_PROBABILITY ? getRandomEmpl(person) : getRandomChild(person);
	}
	private static Person getRandomChild(Person person) {
		...
		return new Child(person.getId(), person.getName(), person.getAdress(), birthDate, garten);
	}
	...
	private static Person getRandomEmpl(Person person) {
		...
		return new Employee(person.getId(), person.getName(), person.getAdress(),
				                 birthDate, company, salary, title);
	}
